import {
  ...
  BOSS_RESPAWN_TIME,
  BUILDINGS,
  DEFAULT_BIOME_DENSITY,
  DEPOSIT_BUILDING,
  ENEMIES,
  ENEMIES_PER_SPAWN_TICK,
...
export class GameEngine {
  ...
  // Densità di spawn scelta dal giocatore per ciascun bioma
  // (basso/medio/alto). Determina il numero massimo di nemici "comuni" che
  // possono vivere contemporaneamente in quel bioma.
  biomeDensity: Record<BiomeId, SpawnDensity> = {
    prateria: DEFAULT_BIOME_DENSITY,
    foresta: DEFAULT_BIOME_DENSITY,
    deserto: DEFAULT_BIOME_DENSITY,
    palude: DEFAULT_BIOME_DENSITY,
    montagne: DEFAULT_BIOME_DENSITY,
    ghiacciaio: DEFAULT_BIOME_DENSITY,
    vulcano: DEFAULT_BIOME_DENSITY,
  };
  // Stato del Boss di ciascun bioma: vivo o in attesa di respawn.
  private bossAlive: Record<BiomeId, boolean> = {
    prateria: true, foresta: true, deserto: true, palude: true,
    montagne: true, ghiacciaio: true, vulcano: true,
  };
  private bossRespawnTimer: Record<BiomeId, number> = {
    prateria: 0, foresta: 0, deserto: 0, palude: 0,
    montagne: 0, ghiacciaio: 0, vulcano: 0,
  };
  ...
  constructor() {
    ...
    // Semina subito molti mostri sparsi su tutti i biomi all'avvio, invece
    // di aspettare che la rampa di spawn periodico li faccia comparire
    // gradualmente (coerente con il cap di densita' "medium" di default).
    for (const b of BIOME_ORDER) {
      const seedCount = Math.floor(SPAWN_DENSITY_CAP[this.biomeDensity[b]] * 0.6);
      for (let i = 0; i < seedCount; i++) this.spawnEnemy(b);
    }
    for (const b of BIOME_ORDER) this.spawnBoss(b);
  ...
  load(): boolean {
    ...
    if (data.bossAlive && typeof data.bossAlive === "object") {
      this.bossAlive = { ...this.bossAlive, ...data.bossAlive };
    } else if (typeof data.bossAlive === "boolean") {
      // Vecchio formato di salvataggio (un solo Boss, la Prateria).
      this.bossAlive.prateria = data.bossAlive;
    }
    if (data.bossRespawnTimer && typeof data.bossRespawnTimer === "object") {
      this.bossRespawnTimer = { ...this.bossRespawnTimer, ...data.bossRespawnTimer };
    } else if (typeof data.bossRespawnTimer === "number") {
      this.bossRespawnTimer.prateria = data.bossRespawnTimer;
    }
    if (data.biomeDensity && typeof data.biomeDensity === "object") {
      for (const id of BIOME_ORDER) {
        const v = data.biomeDensity[id];
        if (v === "low" || v === "medium" || v === "high") this.biomeDensity[id] = v;
      }
    }
  ...
  /** Stato pubblico dei Boss (uno per bioma), utile per la HUD. */
  get bossStatus(): Record<BiomeId, { alive: boolean; respawnIn: number }> {
    const out = {} as Record<BiomeId, { alive: boolean; respawnIn: number }>;
    for (const b of BIOME_ORDER) {
      out[b] = { alive: this.bossAlive[b], respawnIn: Math.max(0, this.bossRespawnTimer[b]) };
    }
    return out;
  }
  ...
  /**
   * Fa comparire un nemico "comune" in un punto casuale su TUTTA
   * l'estensione del bioma indicato (fascia BIOME_ROAM_INNER..OUTER, non piu'
   * solo 1-2 punti fissi come le vecchie SPAWN_AREAS). Se `biome` non e'
   * indicato, ne sceglie uno a caso tra tutti. Il nemico resta poi confinato
   * al proprio bioma per tutta la vita (vedi updateEnemy: spawnBiome).
   */
  spawnEnemy(biome?: BiomeId) {
    const b = biome ?? BIOME_ORDER[Math.floor(Math.random() * BIOME_ORDER.length)];
    const kinds = BIOME_ENEMY_KINDS[b];
    if (!kinds || kinds.length === 0) return;
    const kind = kinds[Math.floor(Math.random() * kinds.length)];
    // Fino a 10 tentativi per evitare di comparire dentro un edificio (in
    // pratica capita solo vicinissimo al bordo del villaggio).
    let pos = randomPointInBiome(b);
    for (let tries = 0; tries < 10 && this.isInsideAnyBuilding(pos); tries++) {
      pos = randomPointInBiome(b);
    }
    this.entities.push(createEnemy(kind, pos, pos, BIOME_ROAM_OUTER, 1, b));
  }

  /** Imposta la densità di spawn (basso/medio/alto) per un singolo bioma. */
  setBiomeDensity(biome: BiomeId, density: SpawnDensity) {
    this.biomeDensity[biome] = density;
  }

  private spawnBoss(biome: BiomeId) {
    this.bossAlive[biome] = true;
    const area = BOSS_AREAS[biome];
    const pos = this.randomPointInArea(area.pos, area.radius * 0.4);
    this.entities.push(
      createEnemy(BIOME_BOSS_KIND[biome], pos, pos, BIOME_ROAM_OUTER, 1, biome),
    );
    this.addLog(`⚠️ Il Boss di ${BIOMES[biome].name} e' apparso nella sua Boss Area!`);
  }
  ...
  private kill(killer: Entity, victim: Entity) {
    ...
    if (victim.faction === "enemy") {
      ...
      if (victim.isBoss && victim.spawnBiome) {
        this.bossAlive[victim.spawnBiome] = false;
        this.bossRespawnTimer[victim.spawnBiome] = BOSS_RESPAWN_TIME;
        this.addLog(`🏆 Boss di ${BIOMES[victim.spawnBiome].name} sconfitto!`);
      }
    }
  ...
  private randomAngleInBiome(biome: BiomeId): number {
    return randomAngleInBiome(biome);
  }
  ...
  private safeZonePoint(biome: BiomeId, radius = VILLAGE.radius + 8): Vec2 {
    ...
    const ang = (idx * BIOME_SECTOR_DEG * Math.PI) / 180;
  ...
  private updateEnemy(e: Entity, dt: number) {
    ...
    // Mostri con un bioma assegnato (tutti quelli generati da spawnEnemy) si
    // confinano a TUTTA l'estensione di quel bioma (inZone), non piu' solo
    // al piccolo cerchio spawnPos/spawnRadius. Chi non ha spawnBiome (es. il
    // Boss, ancora legato alla propria Boss Area puntuale) mantiene il
    // comportamento a cerchio di prima, invariato.
    const biome = e.spawnBiome ?? null;

    // --- Rientro alla Spawn Area / al bioma (leash) ---------------------------
    if (e.ai.state === "returning") {
      const returnTarget = biome ? this.safeZonePoint(biome) : spawnPos;
      moveToward(e, returnTarget, dt, true);
      if (e.health) {
        e.health.hp = Math.min(e.health.maxHp, e.health.hp + LEASH_HEAL_PER_SECOND * dt);
      }
      const arrived = biome
        ? this.inZone(e.transform.pos, biome)
        : dist(e.transform.pos, spawnPos) < 0.5;
      if (arrived) {
        e.ai.state = "idle";
        e.ai.targetId = null;
      }
    ...
    if (e.ai.targetId === p.id) {
      ...
      const overLeash = biome
        ? !this.inZone(e.transform.pos, biome)
        : dist(e.transform.pos, spawnPos) > leashDistance;
    ...
    // --- Nessun bersaglio: vaga liberamente, ma senza mai uscire dal bioma ----
    e.ai.state = "idle";
    e.ai.wanderTimer = (e.ai.wanderTimer ?? 0) - dt;
    if (!e.ai.wanderTarget || e.ai.wanderTimer <= 0) {
      if (biome) this.pickWanderTarget(e, e.transform.pos, WANDER.radius, biome);
      else this.pickWanderTarget(e, spawnPos, spawnRadius);
    }
  ...
  update(dt: number) {
    ...
    // spawn mostri: rispetta la densità scelta per ciascun bioma e un
    // soft-cap globale.
    this.spawnTimer -= dt;
    if (this.spawnTimer <= 0) {
      const totalEnemies = this.entities.filter(
        (e) => e.faction === "enemy" && !e.isBoss,
      ).length;
      if (totalEnemies < MAX_ENEMIES) {
        // Conta i nemici comuni per bioma, poi elenca i biomi ancora sotto
        // il proprio cap. Fa comparire fino a ENEMIES_PER_SPAWN_TICK nemici
        // per tick (uno a bioma scelto a caso tra gli eleggibili), cosi' la
        // popolazione sale in fretta invece che un mostro alla volta.
        const perBiome: Record<BiomeId, number> = {
          prateria: 0, foresta: 0, deserto: 0, palude: 0,
          montagne: 0, ghiacciaio: 0, vulcano: 0,
        };
        for (const e of this.entities) {
          if (e.faction !== "enemy" || e.isBoss) continue;
          perBiome[biomeAt(e.transform.pos)]++;
        }
        let spawnedThisTick = 0;
        while (
          spawnedThisTick < ENEMIES_PER_SPAWN_TICK &&
          this.entities.filter((e) => e.faction === "enemy" && !e.isBoss).length < MAX_ENEMIES
        ) {
          const eligible = BIOME_ORDER.filter(
            (b) => perBiome[b] < SPAWN_DENSITY_CAP[this.biomeDensity[b]],
          );
    ...
    // respawn automatico dei Boss, un timer indipendente per bioma
    for (const b of BIOME_ORDER) {
      if (!this.bossAlive[b]) {
        this.bossRespawnTimer[b] -= dt;
        if (this.bossRespawnTimer[b] <= 0) this.spawnBoss(b);
      }
    }
